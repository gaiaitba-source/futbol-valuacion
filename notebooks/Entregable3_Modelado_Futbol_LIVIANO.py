...
# %% [markdown]
# ---
# ## ⚡ Versión LIVIANA — qué se alivianó y posible impacto
#
# Esta es una **versión optimizada para velocidad** del notebook del Entregable 3. La versión
# completa (`Entregable3_Modelado_Futbol.ipynb`) tarda ~20-25 min en Colab gratis; esta corre en
# **~5-7 min**. El enfoque, las features, el target y las conclusiones son **los mismos** — solo se
# reduce el costo de cómputo. Cambios y su impacto:
#
# | Qué se alivianó | Antes → Ahora | Posible impacto |
# |---|---|---|
# | **Comparación de variantes de caída (3.7)** — la parte más lenta | RF de 150 árboles sobre todo el train → **70 árboles sobre una muestra del 60%** | Solo afecta *qué variante de caída se elige*. Como las variantes daban resultados casi idénticos, el ranking se mantiene. Mínimo riesgo de elegir otra variante; las conclusiones no cambian. |
# | **CV temporal (5)** | RF 200 → **100 árboles** | El R² de validación puede variar ±0.005. No cambia la lectura. |
# | **Random Forest final (6)** | 300 → **150 árboles** | R²/MAE prácticamente iguales (los árboles extra aportan muy poco tras ~150). |
# | **XGBoost puntual (6)** | 500 → **300 árboles** | Cambio despreciable en métricas; la importancia de variables se mantiene. |
# | **Backtest rolling (8)** | XGB 400 → **250 árboles** por año | Las curvas de R² por año quedan casi idénticas. |
# | **Modelo de cuantiles (10)** | 500 → **300 árboles** | El intervalo y su cobertura (~80%) se mantienen; podría moverse ±1-2 puntos de cobertura. |
#
# **En resumen:** menos árboles = un poquito más de varianza en las estimaciones (diferencias de
# centésimas en R²/MAE), pero a cambio de correr 3-4 veces más rápido. Para producción o para la
# entrega "oficial" conviene usar la versión completa; esta es ideal para iterar o para que se
# corra rápido en una demo. Los modelos guardados en `modelos/` sirven igual.
#
# > El resto del notebook es idéntico a la versión completa.
...
for dev in [None, 0.5, 0.3]:
    d = construir(dev)
    tr, _ = split_xy(d)
    # [LIVIANO] La comparación de variantes solo necesita rankearlas: usamos una muestra del 60%
    # del train y menos árboles. No afecta el dataset final, solo qué variante elegimos.
    tr_s = tr.sample(frac=0.6, random_state=42) if len(tr) > 80000 else tr
    Xtr, ytr = tr_s[FEATURES].fillna(0), tr_s[TARGET]
    rf = RandomForestRegressor(n_estimators=70, max_depth=12, min_samples_leaf=20, random_state=42, n_jobs=-1)
    rf.fit(Xtr, ytr)
...
cv_rf = cross_val_score(RandomForestRegressor(n_estimators=100, max_depth=14, min_samples_leaf=20,
...
xgb_point = xgb.XGBRegressor(n_estimators=300, max_depth=6, learning_rate=0.05, subsample=0.8,
...
for A in [2021, 2022, 2023, 2024]:
    ...
    m = xgb.XGBRegressor(n_estimators=250, max_depth=6, learning_rate=0.05, subsample=0.8,
                         colsample_bytree=0.8, random_state=42, tree_method='hist')
...
# %%
# [LIVIANO] Guardamos en una carpeta aparte para no pisar los modelos de la versión completa.
MODELO_PATH = Path(DATA_PATH).resolve().parent.parent / 'modelos_liviano'
